...
- [ ] 修复/补齐 `.test.tsx` 渲染测试入口，使组件测试可被实际运行：
...
- [ ] 创建 `EntityDetailPopup` 组件，props: `{ client: GameClient; entity: RoomEntity | null }`
- [ ] 使用 `PopupPanel` 作为布局基座，保持小型目标上下文弹窗形态
  - `title`: `entity().name`
  - `borderColor`: `THEME.focus`
  - `width`: 使用固定或内容约束宽度，避免覆盖事件日志或出口区域
- [ ] 内容区：显示 `entity().typeLabel`（如果有）和 `entity().description`（如果有）；描述较长时使用 `wrapMode="word"` 或局部滚动
- [ ] 操作区：用顶部分隔线与内容区分开，展示 `getEntityActions` 返回的操作列表，用 `KeyHint` 展示，点击执行 `action.run` 并清除 `selectedEntityId`
- [ ] 加 `hasActiveRequest()` 加载态：显示 `LoadingHint` ，隐藏操作列表

## Component: `src/tui/components/index.ts` (modify)

- [ ] 新增导出 `EntityDetailPopup` from `./entity-detail-popup.tsx`

## Component: `src/tui/panels/room/room-panel.tsx` (modify)

- [ ] import `EntityDetailPopup` from `../../components/index.ts`
- [ ] 将当前 `<TargetActionPopup client={...} entity={...} />` 替换为按类型分流：
...
- [ ] **新增** `src/__tests__/entity-detail-popup.test.tsx`
...
- [ ] **新增** `src/__tests__/room-panel.test.tsx`
  - item 实体选中 → 渲染 `EntityDetailPopup`
  - non-item 实体选中（npc 无 talk 能力）→ 渲染 `TargetActionPopup`

## Manual Checks

- [ ] `npm run dev:tui` — 房间中选择物品确认弹出 `EntityDetailPopup`，显示"物品"标签+描述+操作
- [ ] 选择 NPC（无 talk 能力）确认仍弹出 `TargetActionPopup`

## Verification

- [ ] Run `npm run lint` (biome check + tsc --noEmit)
- [ ] Run `npm test`
- [ ] Run the `.test.tsx` rendering test entry chosen above against:
...
- [ ] Run `npm run lint` covers depcruise — confirm no tui-no-direct-engine-import violations
