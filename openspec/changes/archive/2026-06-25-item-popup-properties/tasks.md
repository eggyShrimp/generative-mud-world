...
- [x] 先完成 `tui-entity-detail-popup`
- [x] 确认 `.test.tsx` 渲染测试入口可执行；本变更不能只依赖 `npm test` 验证组件渲染

## Protocol: `src/shared/protocol.ts` (modify)

- [x] `RoomEntity` 接口新增 `properties?: Record<string, unknown>` 字段

## Engine: `src/engine/capability-provider.ts` (modify)

- [x] `getRoomEntitiesInfo()` 对 `e.type === "item"` 的实体附加 `properties`：从 `ItemEntity.properties` 透传

## TUI: `src/tui/components/entity-detail-popup.tsx` (modify)

- [x] import `formatItemProperties` from `../../../shared/item-format.ts`
- [x] 描述 (`entity().description`) 下方新增属性展示行：
...
- [x] **更新** `src/__tests__/entity-detail-popup.test.tsx`
...
- [x] **更新** `src/__tests__/engine.test.ts` 中现有 `getRoomEntitiesInfo` 覆盖
...
- [x] `npm run dev:tui` — 房间中选择有属性的物品（如武器），确认弹窗展示属性行
- [x] 房间中选择无属性物品，确认弹窗回退正常，不崩溃
- [x] 背包详情（`i`）属性展示不受影响
...
- [x] Run the `.test.tsx` rendering test entry against `src/__tests__/entity-detail-popup.test.tsx`
- [x] Run `npx biome check src/`
